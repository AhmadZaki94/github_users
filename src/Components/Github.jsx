...
import { Image,Box,Text, Center, Button, Input, Heading } from '@chakra-ui/react';
...
export const Github = () => {
  ...
  return (
    <div>
      ...
      <div>
        <Button variant='solid' coloScheme='teal'
          disabled={page === 1}
          onClick={() => {
            setPage(page - 1);
          }}
...
const SearchBox = ({ handleClick }) => {
  ...
  return (
    <div>
      <Box>
      <Input htmlSize={20} width='auto' border={'2px solid grey'}
        type="text"
...
const GithubCard = ({ avatar_url, login }) => {
  return (
    <Center style={{ display: "flex", gap: "2rem", marginBottom: '15px', marginTop: '15px' }}>
